...
namespace BindOpen.Framework.Core.Extensions.Items.Libraries
{
    ...
    public static partial class LibraryIndexer
    {
            ...
            ILog log = null)
        {
            ...
            var types = assembly.GetTypes().Where(p => typeof(Task).IsAssignableFrom(p));
            foreach(Type type in types)
            {
                ...
                {
                    definition.InputSpecification.Add(ElementSpecFactory.Create(property.PropertyType));
                }

                foreach (PropertyInfo property in type.GetProperties().Where(p => p.GetCustomAttribute(typeof(TaskOutputAttribute)) != null))
                {
                    definition.OutputSpecification.Add(ElementSpecFactory.Create(property.PropertyType));
                }
