        ...
        switch (dbItem.Kind)
        {
            ...
            case BdoConditionKind.Composite:
                if (poco is IBdoCompositeCondition compositeCondition)
                {
                    ...
                    dbItem.Conditions ??= [];
                    dbItem.Conditions.RemoveAll(q => compositeCondition.Conditions?.Any(p => p?.Identifier == q?.Identifier) != true);

                    if (compositeCondition?.Conditions?.Count > 0)
                    {
                        foreach (var subCondition in compositeCondition.Conditions)
                        {
